...
string DATA_FILE = "./made_files/cleaned_data.csv";
string TESTING_FILE = "./test_files/earth_test.csv";
string ERRORS_FILE = "./made_files/saved_errors.csv";
string SIMULATION_FILE = "./made_files/simulated_data.csv";
double WIGGLE_ROOM = 1.3; // Multiplying max_vec by this number so that orbits don't behave weird if for a sample orbit the number is greater than max_vec
double DELTA_SCALE = 10.0; // To make sure that deltas aren't being reduced to 0 to cheat error
...
void get_batch(const vector<OrbitState> &data, const vector<int> &indices, int batch_idx, int batch_size, MatrixXd &X, MatrixXd &Y){
    ...
    // Wrap around if global_idx is outside data size
        ...
        Y(0, i) = (data[global_idx + 1].x - data[global_idx].x) * DELTA_SCALE;
        Y(1, i) = (data[global_idx + 1].y - data[global_idx].y) * DELTA_SCALE;
        Y(2, i) = (data[global_idx + 1].z - data[global_idx].z) * DELTA_SCALE;
        Y(3, i) = (data[global_idx + 1].vx - data[global_idx].vx) * DELTA_SCALE;
        Y(4, i) = (data[global_idx + 1].vy - data[global_idx].vy) * DELTA_SCALE;
        Y(5, i) = (data[global_idx + 1].vz - data[global_idx].vz) * DELTA_SCALE;
    }
...
vector<OrbitState> simulate_data(const string &filename, const OrbitState &start_state, int steps, double dt,const MatrixXd &w1, const VectorXd &b1, const MatrixXd &w2, const VectorXd &b2, const VectorXd &max_vec){
    ...
    // Forward Passes
        change = (w1*X).colwise() + b1;
        change = activation_tanh(change);
        change = (w2*change).colwise() + b2;
        
        X += (change / DELTA_SCALE);
    // Saving Data
        VectorXd Z = X;
...
// Normalising Data
    ...
    for(int i=0; i<test_set.size()-1; i++){
        ...
        Y(0,0) = (test_set[i+1].x - test_set[i].x) * DELTA_SCALE;
        Y(1,0) = (test_set[i+1].y - test_set[i].y) * DELTA_SCALE;
        Y(2,0) = (test_set[i+1].z - test_set[i].z) * DELTA_SCALE;
        Y(3,0) = (test_set[i+1].vx - test_set[i].vx) * DELTA_SCALE;
        Y(4,0) = (test_set[i+1].vy - test_set[i].vy) * DELTA_SCALE;
        Y(5,0) = (test_set[i+1].vz - test_set[i].vz) * DELTA_SCALE;
...
// Initialising Weights
    ...
    for(int i=0; i<NUM_EPOCHS; i++){
        ...
        // Clamping gradients
            dw1 = dw1.array().max(-1.0).min(1.0).matrix();
            dw2 = dw2.array().max(-1.0).min(1.0).matrix();
            db1 = db1.array().max(-1.0).min(1.0).matrix();
            db2 = db2.array().max(-1.0).min(1.0).matrix();
